...
#define LCG_M 0xFFFFFFFF // 2^32, to ensure wrap-around
#define TIME_SLICE 1
#define MAXTIME 30
...
// struct proc *random_lottery(int total_tickets)
// {
//   int value = rand_at_max(total_tickets) + 1;
//   int sum = 0;
//   struct proc *winner = 0;
//   int winner_tickets = 0;
//   for (struct proc *p = 0; p < &proc[NPROC]; p++)
//   {
//     if (p->state != RUNNABLE)
//       continue;
//     sum += p->tickets;
...
//     {
//       if (p->state != RUNNABLE || p->tickets != winner_tickets)
//         continue;
//       if (p->arrival_time < winner->arrival_time)
//         winner = p;
//     }
//   }
//   return winner;
...
struct child_info
{
  ...
  int syscall_count[32]; // Syscall counts (assuming 32 syscalls)
};
...
struct proc *multique[4][NPROC];
int sizeofq[4] = {0};
int timesliceq[4] = {1, 4, 8, 16};
  ...
  for (;;)
  {
...
#ifdef LBS
    struct proc *p;
    // Step 1: Calculate the total number of tickets from all RUNNABLE processes.
    int total_tickets = 0;
    for (p = proc; p < &proc[NPROC]; p++)
    {
      acquire(&p->lock);
    ...
    if (total_tickets > 0)
    {
      ...
      if (winner != 0)
      {
        for (struct proc *p = proc; p < &proc[NPROC]; p++)
        {
          if (p->state != RUNNABLE || p->tickets != winner->tickets)
            continue;
          if (p->arrival_time < winner->arrival_time)
            winner = p;
        }
    ...
    for (int level = 0; level <= 3; level++)
    {
      for (int j = 0; j < sizeofq[level]; j++)
      {
        if(multique[level][j]->state==RUNNABLE) {
          p = multique[level][j]; // Get the first process from the highest-priority queue
          // dequeue(this_proc, level);      // Dequeue the process for scheduling
          flag=1;
          break;
        }
      }
